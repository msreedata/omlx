        ...
        async archiveSelectedModels() {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    if (successCount > 0) {
                        this.hfSuccess = window.t('js.success.archived_models').replace('{count}', successCount);
                    }
                    ...
                    const data = await response.json();
                    this.hfError = data.detail || window.t('js.error.archive_model_failed');
                }
            } catch (err) {
                console.error('Failed to archive models:', err);
                this.hfError = window.t('js.error.archive_model_connection');
            } finally {
                this.archiving = false;
                this.hfArchiveConfirm = null;
                setTimeout(() => { this.hfError = ''; this.hfSuccess = ''; }, 5000);
            }
        },

        async unarchiveSelectedModels() {
            if (this.selectedArchivedModels.length === 0) return;

            // Show confirmation dialog
            const count = this.selectedArchivedModels.length;
            const confirmed = await new Promise((resolve) => {
                this.hfUnarchiveConfirm = {
                    action: 'unarchive',
                    count: count,
                    resolve: resolve
                };
            ...
            try {
                const response = await fetch('/admin/api/hf/models/unarchive', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ model_names: this.selectedArchivedModels }),
                });
                ...
                if (response.ok) {
                    ...
                    if (successCount > 0) {
                        this.hfSuccess = window.t('js.success.unarchived_models').replace('{count}', successCount);
                    }
                    if (failCount > 0 && data.failed) {
                        const failedNames = data.failed.map(f => f.name).join(', ');
                    ...
                    const data = await response.json();
                    this.hfError = data.detail || window.t('js.error.unarchive_model_failed');
                }
            } catch (err) {
                console.error('Failed to unarchive models:', err);
                this.hfError = window.t('js.error.unarchive_model_connection');
            } finally {
                this.unarchiving = false;
                this.hfUnarchiveConfirm = null;
                setTimeout(() => { this.hfError = ''; this.hfSuccess = ''; }, 5000);
            }
        },
